...
| AZW3/KF8 (HUFF/CDIC) | MobiReader | ✅ 基本可用 | 2026-07-12 修复：BigInt→byte-level 位读取 + trailing bytes 剥离 + 拼接解码 |
| MOBI v7 (无压缩) | MobiReader | ⚠️ 待实测 | 2026-07-12 添加 compression=1 分支 |
...
1. ~~**HUFF/CDIC 压缩的 AZW3 文件解析失败**~~ ✅ 已修复 — trailing bytes 剥离 + 拼接解码是关键
2. **MOBI 解析稳定性** — PalmDOC 文件偶尔有 HTML 标签断裂（如 `<table>` 变成 `lass="...">`），可能由 trailing bytes 过度裁剪引起
3. **CBR 漫画格式未实测** — 没有测试文件，RAR 解压逻辑未经验证
4. **HUFF/CDIC 剩余 ~5% 乱码** — 多文档拼接处和长 Huffman 码边缘 case 有零星乱码

## 2026-07-12 修复记录（完整）

1. **HUFF/CDIC Huffman 解码器重写** — BigInt `getBigUint64` → byte-level `readBitsMSB`，避免位读取精度问题
2. **CDIC 偏移验证** — 确认 16-byte header（含 `length` 字段）是正确的 CDIC 格式
3. **Trailing bytes 剥离（关键）** — HUFF/CDIC 文本记录末端的 trailing data 在解码前必须像 PalmDOC 一样剥离，否则 bit 流被污染
4. **拼接解码** — 先剥离 trailing bytes 再拼接所有记录为连续 Huffman bit 流，一次性解码
5. **MOBI v7 无压缩支持** — 添加 `compression === 1` 分支
6. **多文档 join** — 移除 boundary-based KF8 单选，改为 join 所有 bodies

## 待实现功能（按优先级）

1. ~~HUFF/CDIC 压缩的 MOBI/AZW3 正确解析~~ ✅ 2026-07-12 已重写，待实测
2. ~~MOBI v7 无压缩支持~~ ✅ 2026-07-12 已添加
3. 阅读主题系统（亮/纸/护眼/暗/自然 + 字体选择）
4. 书籍封面提取
5. 豆瓣评分集成
6. CBR 漫画完善
7. DJVU 支持
